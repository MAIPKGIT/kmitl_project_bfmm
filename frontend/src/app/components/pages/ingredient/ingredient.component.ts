...
export class IngredientComponent implements OnInit {
  ...
  createIngredient() {
    if (this.selectedImageFile) {
      this.newIngredientData.Ingredients_image = this.selectedImageFile.name;
    }
  
    const jsonData = { ...this.newIngredientData };
  
    console.log("JSON ที่จะส่งไปยัง API (POST):", JSON.stringify(jsonData, null, 2));
  
    this.service.createIngredient(jsonData).subscribe(() => {
      Swal.fire('สำเร็จ', 'เพิ่มวัตถุดิบแล้ว', 'success');
      this.loadIngredients();
      this.newIngredientData = {
        Ingredients_name: '', Ingredients_des: '', Ingredients_image: '', main_stock: 0, sub_stock: 0, unit: 'ml'
      };
  ...
  updateIngredient() {
    if (this.selectedEditImageFile) {
      this.editIngredientData.Ingredients_image = this.selectedEditImageFile.name;
    }
  
    const jsonData = { ...this.editIngredientData };
  
    console.log("JSON ที่จะส่งไปยัง API (PUT):", JSON.stringify(jsonData, null, 2));
  
    this.service.updateIngredient(this.editIngredientData.Ingredients_id, jsonData).subscribe({
      next: () => {
      ...
      error: (err) => {
        ...
        Swal.fire('ผิดพลาด', err.error.message || 'เกิดข้อผิดพลาดในการอัปเดต', 'error');
      }
    });
  }  
